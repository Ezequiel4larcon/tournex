...
import { Gamepad2, Trophy, Plus, Shield, Grid3x3, Swords } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      // Filtrar torneos donde el usuario es owner
      setModeratedTournaments(allTournaments.filter(t => {
        const ownerId = t.owner?._id || t.owner;
        return ownerId && userId && ownerId === userId;
      }));

      // Filtrar torneos donde el usuario participa (no owner)
      const participating = allTournaments.filter(t => {
        const ownerId = t.owner?._id || t.owner;
        const isNotOwner = !ownerId || !userId || ownerId !== userId;
        const isParticipant = t.participants?.some(p => (p.player?._id || p.player) === userId);
      ...
      for (const tournament of participatingTournaments) {
        try {
          ...
          userMatches.forEach(match => {
            match.tournamentInfo = { _id: tournament._id, name: tournament.name, game: tournament.game };
          });
  ...
  const activeTournaments = myTournaments.filter(t => t.status === 'in_progress' || t.status === 'registration_open');
  const activeModerated = moderatedTournaments.filter(t => t.status === 'in_progress' || t.status === 'registration_open' || t.status === 'pending');

  if (loading) {
    return <div className="min-h-screen bg-background flex items-center justify-center"><Spinner text="Cargando dashboard..." size="lg" /></div>;
  }
  ...
  return (
    <main className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        {/* Welcome Section */}
        <div className="mb-16">
          <h2 className="text-4xl font-bold text-foreground mb-3">Bienvenido, {user?.username || 'Jugador'}</h2>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Link to={`/tournaments/${match.tournamentInfo?._id}/bracket`}>
                          <Button variant="outline" size="sm" className="text-xs hover:border-primary transition-colors">Ver Bracket</Button>
                        </Link>
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex gap-4 mb-8 border-b border-border">
            <button
              ...
              className={`px-4 py-3 font-medium transition-all duration-200 ${
                activeTab === 'playing' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'
              }`}
            >
              <div className="flex items-center gap-2">
                <Trophy className="w-4 h-4" /> Participando ({myTournaments.length})
              </div>
            </button>
            <button
              onClick={() => setActiveTab('moderating')}
              className={`px-4 py-2 font-medium transition-colors ${
                activeTab === 'moderating' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'
              }`}
            >
              <div className="flex items-center gap-2">
                <Shield className="w-4 h-4" /> Moderando ({moderatedTournaments.length})
              </div>
            </button>
          </div>
          ...
          <div className="space-y-4">
            {activeTab === 'playing' && myTournaments.length === 0 && (
              <div className="bg-card/50 backdrop-blur-sm border border-border rounded-xl p-12 text-center">
                ...
                <Link to="/tournaments"><Button variant="outline" className="hover:border-primary transition-colors">Explorar Torneos</Button></Link>
              </div>
            )}
            ...
            {activeTab === 'moderating' && moderatedTournaments.length === 0 && (
              <div className="bg-card/50 backdrop-blur-sm border border-border rounded-xl p-12 text-center">
                ...
                <Link to="/tournaments/create"><Button variant="outline" className="hover:border-primary transition-colors">Crear Torneo</Button></Link>
              </div>
            )}

            {activeTab === 'playing' && myTournaments.map((tournament) => (
              <TournamentCard key={tournament._id} tournament={tournament} variant="participant" showBracketButton />
            ))}

            {activeTab === 'moderating' && moderatedTournaments.map((tournament) => (
              <TournamentCard key={tournament._id} tournament={tournament} variant="moderator" showBracketButton />
            ))}
          </div>
        </div>
        ...
        <div className="bg-card/50 backdrop-blur-sm border border-border rounded-xl p-8 hover:border-primary transition-all duration-300">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <Link to="/tournaments"><Button className="bg-primary hover:bg-primary/90 transition-colors whitespace-nowrap">Ver Todos los Torneos</Button></Link>
          </div>
        </div>
      </div>
    </main>
